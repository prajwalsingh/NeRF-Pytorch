...
class NerfComponents:
	def __init__(self, height, width, batch_size, num_samples, pos_enc_dim, dir_enc_dim, device='cuda'):
		self.height = height
		self.width  = width
		self.device = device
	...
	def sampling_rays(self, ray_origin, ray_direction, near, far, random_sampling=True):
		...
		z_vals = torch.tile(torch.unsqueeze(z_vals, dim=0), [ray_direction.shape[0], 1]) # N_rays x N_samples
		...
		if random_sampling:
			...
			# t_vals = t_vals + noise
			mids = .5 * (z_vals[..., 1:] + z_vals[..., :-1]) # N_rays x (N_samples-1)
			upper = torch.concat([mids, z_vals[..., -1:]], dim=-1) # N_rays x N_samples
			lower = torch.concat([z_vals[..., :1], mids], dim=-1) # N_rays x N_samples
			# stratified samples in those intervals
			t_rand = torch.rand(z_vals.shape).to(config.device) # N_rays x N_samples
			z_vals = lower + (upper - lower) * t_rand # N_rays x N_samples
	...
	def inverse_transform_sampling(self, t_vals_mid, weights):
		...
		# cdf = torch.concat([torch.zeros_like(cdf[..., :1]).to(self.device), cdf], dim=-2)
	...
	def render_rgb_depth(self, rgb, density, rays_d, t_vals, random_sampling=True):
		...
		# N_rays x N_samples
		delta = delta * torch.linalg.norm(torch.unsqueeze(rays_d, dim=1), ord=2, dim=-1) # Euclidean Norm

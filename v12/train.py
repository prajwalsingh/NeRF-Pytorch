...
if __name__ == '__main__':
	...
	base_c2wMatrix, base_direction, base_near, base_far, base_focal = base_c2wMatrix.to(config.device),\
															base_direction.to(config.device),\
															base_near.to(config.device),\
															base_far.to(config.device),\
															base_focal.to(config.device)
	base_direction = torch.reshape(base_direction, (config.batch_size, -1, 3))
	...
	for epoch in range(START_EPOCH, END_EPOCH):
		...
		for idx, (image, c2wMatrix, focal, direction, near, far) in enumerate(tq, start=1):
			...
			with torch.no_grad():
				...
				focal, direction = focal.to(config.device), direction.to(config.device)
				near,  far       = near.to(config.device), far.to(config.device)
				image            = torch.permute(image, (0, 2, 3, 1))
				image            = torch.squeeze(image.reshape(config.batch_size, -1, 3), dim=0)
				direction        = direction.reshape(config.batch_size, -1, 3)
			...
			del rgb_coarse, depth_map_coarse, weights_coarse, fine_rays, t_vals_fine, rgb, density, rgb_fine, depth_map_fine, weights_fine, loss, ray_direction_c, ray_direction_f, direction, near, far, ray_origin, ray_direction

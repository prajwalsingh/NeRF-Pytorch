...
class NerfComponents:
	...
	def get_rays(self, camera_matrix, direction):
		...
		rotation_matrix_c  = torch.unsqueeze(rotation_matrix, dim=1)
		...
		# Ray origin
		ray_origin         = torch.tile(torch.unsqueeze(translation, dim=-2), (1, ray_direction.shape[1], 1))
	...
	def sampling_rays(self, ray_origin, ray_direction, near, far, random_sampling=True):
		...
		# t_vals = torch.squeeze(t_vals, dim=0)
	...
	def render_rgb_depth(self, rgb, density, rays, t_vals, random_sampling=True):

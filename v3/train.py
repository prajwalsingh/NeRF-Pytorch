...
from dataloader_nerf360 import NerfDataLoader360
...
if __name__ == '__main__':
	...
	#########################################################################################
	train_dataloader = DataLoader(NerfDataLoader360(jsonPath=config.train_json_path,\
	...
	# val_dataloader = DataLoader(NerfDataLoader360(jsonPath=config.val_json_path,\
	# 									   datasetPath=config.image_path,\
	# 									   imageHeight=config.image_height,\
	# 									   imageWidth=config.image_width),\
	# 									   batch_size=config.batch_size, shuffle=True,\
	# 									   num_workers=8, pin_memory=True, drop_last=True)
	base_image, base_c2wMatrix, base_focal, base_direction, base_near, base_far = next(iter(train_dataloader))
	base_image, base_c2wMatrix, base_direction, base_near, base_far = base_image.to(config.device),\
													 base_c2wMatrix.to(config.device), base_direction.to(config.device),\
													 base_near.to(config.device), base_far.to(config.device)
	...
	for epoch in range(START_EPOCH, END_EPOCH):
		...
		for idx, (image, c2wMatrix, focal, direction, near, far) in enumerate(tq, start=1):
			...
			image, c2wMatrix, direction, near, far = image.to(config.device),\
													 c2wMatrix.to(config.device), direction.to(config.device),\
													 near.to(config.device), far.to(config.device)

			rays, t_vals = nerf_comp.sampling_rays(direction=direction, near=near, far=far, camera_matrix=c2wMatrix, random_sampling=True)
			...
			fine_rays, t_vals_fine = nerf_comp.sampling_fine_rays(direction=direction, camera_matrix=c2wMatrix, t_vals=t_vals, weights=weights)
			...
			del image, c2wMatrix, rays, t_vals, prediction, rgb_coarse, weights, fine_rays, t_vals_fine, rgb_fine, loss, direction, near, far
		...
		# 		tq.set_description('E: {}, VL: {:0.3f}, VPSNR: {:0.3f}'.format(epoch, sum(val_loss_tracker)/len(val_loss_tracker), sum(val_psnr_tracker)/len(val_psnr_tracker)))
			...
			rays, t_vals = nerf_comp.sampling_rays(direction=base_direction, near=base_near, far=base_far, camera_matrix=base_c2wMatrix, random_sampling=True)
			prediction   = nerfnet_coarse(rays)
			prediction   = torch.reshape(prediction,\
										(config.batch_size,\
										config.image_height,\
			...
			fine_rays, t_vals_fine  = nerf_comp.sampling_fine_rays(direction=base_direction, camera_matrix=base_c2wMatrix, t_vals=t_vals, weights=weights)
			prediction   = nerfnet_fine(fine_rays)
			prediction   = torch.reshape(prediction,\
										(config.batch_size,\
										config.image_height,\

...
class NerfComponents:
	...
	def get_rays(self, direction, camera_matrix):
	...
	def sampling_rays(self, direction, near, far, camera_matrix, random_sampling=True):

		ray_origin, ray_direction = self.get_rays(direction, camera_matrix)

		# Compute 3D query points
		# r(t) = o + td -> we are buildin t here [x,y,z] of raidance
		# this is discrete sampling
		t_vals = torch.linspace(near[0], far[0], self.num_samples).to(self.device)
		...
		if random_sampling:
			 ...
			 noise  = torch.rand(size=shape).to(self.device) * ((far - near) / self.num_samples)
			 t_vals = t_vals + noise
	...
	def sampling_fine_rays(self, direction, camera_matrix, t_vals, weights):
	...
	def sub_batching(self, rays, chunk_size=4096):

...
class NerfDataLoader(Dataset):
	def __init__(self, jsonPath, imageWidth=128, imageHeight=128, datasetPath='', dataType='train'):
		...
		for frame in jsonData['frames']:
			...
			self.fov.append(jsonData['camera_angle_x'])
	...
	def __getitem__(self, idx):
		...
		downscale = W / self.imageWidth
		focal     = ((W/2.0)/(math.tan(self.fov[idx]/2.0)))/downscale # f =  (p/2) / tan(theta/2)
		return (image, c2w, focal)

...
class NerfComponents:
	...
	def sampling_rays(self, camera_matrix, random_sampling=True):
		...
		dirc   = torch.reshape(dirc, (self.batch_size, -1,  self.num_samples, 3))
		dirc   = self.encode_position(dirc, self.dir_enc_dim)

		t_vals = torch.reshape(t_vals, (self.batch_size, -1, self.num_samples))

		# return (rays, t_vals)
		return (xyz, dirc, t_vals)
	...
	def inverse_transform_sampling(self, t_vals_mid, weights):
		...
		t_vals_mid = torch.concat([t_vals_mid, torch.ones(size=(self.batch_size, t_vals_mid.shape[-2], 1)).to(self.device)*(torch.max(t_vals_mid)+1e-5)], dim=-1)
		...
		uniform_sample = torch.rand(size=(self.batch_size, pdf.shape[1], self.num_samples, 1)).to(self.device)
	...
	def sampling_fine_rays(self, camera_matrix, t_vals, weights, idx, chunk_size):
		
		# calculating camera origin and ray direction
		ray_origin, ray_direction = self.get_rays(camera_matrix)

		ray_origin, ray_direction = ray_origin.reshape(self.batch_size, -1, 3),\
									ray_direction.reshape(self.batch_size, -1, 3)

		ray_origin, ray_direction = ray_origin[:, idx*chunk_size:idx*chunk_size+chunk_size, :],\
									ray_direction[:, idx*chunk_size:idx*chunk_size+chunk_size, :]
		...
		xyz   = torch.unsqueeze(ray_origin, dim=-2) + torch.unsqueeze(t_vals_fine, dim=-1)
		dirc  = torch.unsqueeze(ray_origin, dim=-2) +\
				torch.unsqueeze(ray_direction, dim=-2) * torch.unsqueeze(t_vals_fine, dim=-1)
		...
		return (xyz, dirc, t_vals_fine)


	# def render_rgb_depth(self, prediction, rays, t_vals, random_sampling=True):
	def render_rgb_depth(self, density, rgb, xyz, dirc, t_vals, random_sampling=True):
		...
		if random_sampling:
			...
			delta = torch.concat([
									delta,
									torch.ones(size=(self.batch_size, delta.shape[1], 1)).to(self.device) * 1e10
								], dim=-1)
	...
	# 	rays_sub   = [rays[:, i:i+chunk_size] for i in range(0, rays.shape[1], chunk_size)]
	# 	t_vals_sub = [t_vals[:, i:i+chunk_size] for i in range(0, t_vals.shape[1], chunk_size)]
	# 	image_sub  = [image[:, i:i+chunk_size] for i in range(0, rays.shape[1], chunk_size)]
		
	# 	return rays_sub, t_vals_sub, image_sub

	def sub_batching(self, xyz, dirc, t_vals, image, chunk_size=4096):
		xyz_sub    = [xyz[:, i:i+chunk_size] for i in range(0, xyz.shape[1], chunk_size)]
		dirc_sub   = [dirc[:, i:i+chunk_size] for i in range(0, dirc.shape[1], chunk_size)]
		t_vals_sub = [t_vals[:, i:i+chunk_size] for i in range(0, t_vals.shape[1], chunk_size)]
		image_sub  = [image[:, i:i+chunk_size] for i in range(0, image.shape[1], chunk_size)]
		
		return xyz_sub, dirc_sub, t_vals_sub, image_sub
